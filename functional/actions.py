...
from keyboards.button import kb_main, kb_main2, kb_cancel
...
async def upload(message: types.Message):
    ...
    await message.reply("Send 'Number for order' please")#, kb_cancel)
    people.clear
...
async def upnumber(message: types.Message, state: FSMContext):
    ...
    await message.reply("Send address please")#, kb_cancel)
...
async def upaddress(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    people.append(message.text)
    await message.reply("Send your name please")#, kb_cancel)
...
async def upname(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    people.append(message.text)
    await message.reply("Send number of your phone please")#, kb_cancel)
    
async def upphone(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['phone'] = message.text
    people.append(message.text)
    await message.reply("Send your wishes please")#, kb_cancel)
...
async def feedback(message: types.Message):
    ...
    await message.reply("Write your review please")#, kb_cancel)
...
async def phones(message: types.Message):
    help = ["78488487","464841848",3135484354]
    await message.reply(str(help[0])+"\n"+str(help[1])+"\n"+str(help[2]), reply_markup=kb_main2)
...
def register_message_handler(dp: Dispatcher):
    ...
    dp.register_message_handler(dopbutton, lambda message: message.text.startswith("Menu"))
    dp.register_message_handler(McDon, lambda message: message.text.startswith("McDonalds"))
    dp.register_message_handler(rest, lambda message: message.text.startswith("Restaurant"))
    ...
    dp.register_message_handler(feedback, Text(equals='Give feedback', ignore_case=True), state=None)
